...
def plot_scatter(df, save_path=None):
    ...
    df_af3 = df[df['better'] == "af3"]

    plt.figure(figsize=(6, 6))

    # Plot quantum points with circle marker and af3 points with triangle marker
    plt.scatter(df_quantum['quantum'], df_quantum['af3'], s=150, marker='o', color='tab:blue', edgecolors="k",
                linewidths=1, alpha=0.95, label="Better: Quantum")
    plt.scatter(df_af3['quantum'], df_af3['af3'], s=150, marker='^', color='tab:pink', edgecolors="k", linewidths=1,
                alpha=0.95, label="Better: AF3")

    x_min = df['quantum'].min() - 0.5
    x_max = df['quantum'].max() + 0.5
    y_min = df['af3'].min() - 0.5
    y_max = df['af3'].max() + 0.5
    ...
    plt.ylabel("AF3", fontsize=14, fontname="Arial")
    plt.title("Compare RMSD: Quantum vs AF3", fontsize=16, fontname="Arial")
...
def main():
    group = 'M'
    data_file = f"{group}/rmsd_compare.txt"
    df = load_data(data_file)
    print(df)
    output_plot_folder = f"plots/{group}"
    os.makedirs(output_plot_folder, exist_ok=True)
    save_path = os.path.join(output_plot_folder, f"rmsd_af3_{group}.png")
